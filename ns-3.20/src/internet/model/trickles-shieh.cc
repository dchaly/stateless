...
std::clog << "\n";
    ...
    TricklesShieh::TricklesShieh ():TricklesSocketBase(), m_tcpBase(SequenceNumber32(0)), m_cwnd(ShiehInitialCwnd), m_ssthresh(ShiehInitialSsthresh), m_started(false)
    {
        NS_LOG_FUNCTION_NOARGS ();
    }
    
    TricklesShieh::TricklesShieh(const TricklesShieh &sock)
    : TricklesSocketBase(sock), m_tcpBase(SequenceNumber32(0)), m_cwnd(ShiehInitialCwnd), m_ssthresh(ShiehInitialSsthresh), m_started(false) {
    ...
    void TricklesShieh::NewRequest() {
        ...
        if (!m_started) {
            ...
            m_ssthresh = ShiehInitialSsthresh;
            if (m_reqDataSize>=(m_cwnd*m_segSize)) {
                m_started = true;
                for (uint32_t i=0; i<m_cwnd; i++) {
                    TricklesHeader trh;
                    ...
                    m_RcvdRequests.AddBlock(SequenceNumber32(i+1),SequenceNumber32(i+2));
                    //trh.SetSacks(m_RcvdRequests);
                    Ptr<Packet> p = Create<Packet> ();
                    p->AddHeader(tsh);
                    p->AddHeader(trh);
                    //MY_LOG_TRICKLES_PACKET(p);
                    m_retxEvent = Simulator::Schedule(m_rtt->GetMinRto(), &TricklesShieh::ReTxTimeout, this);
                    // Физически отправить пакет
...
//                MY_LOG_TRICKLES_PACKET(p);
    ...
    void TricklesShieh::SendDelayedPackets() {
        NS_LOG_FUNCTION (this);
        while (m_delayed.size()>0) {
            Ptr<Packet> p = m_delayed.front();
            m_delayed.pop_front();
            //MY_LOG_TRICKLES_PACKET(p);
            m_retxEvent.Cancel();
