...
#include "trickles-shieh-header.h"
...
NS_LOG_COMPONENT_DEFINE ("TricklesL4Protocol");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (TricklesL4Protocol);
...
/* The number is taken from unassigned numbers */
const uint8_t TricklesL4Protocol::PROT_NUMBER = 144;

TypeId 
TricklesL4Protocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TricklesL4Protocol")
    .SetParent<IpL4Protocol> ()
    .AddConstructor<TricklesL4Protocol> ()
    .AddAttribute ("RttEstimatorType",
                   "Type of RttEstimator objects.",
                   TypeIdValue (RttMeanDeviation::GetTypeId ()),
                   MakeTypeIdAccessor (&TricklesL4Protocol::m_rttTypeId),
                   MakeTypeIdChecker ())
    .AddAttribute ("SocketType",
                   "Socket type of Trickles objects.",
                   TypeIdValue (TricklesSocketBase::GetTypeId ()),
                   MakeTypeIdAccessor (&TricklesL4Protocol::m_socketTypeId),
                   MakeTypeIdChecker ())
    .AddAttribute ("SocketList", "The list of sockets associated to this protocol.",
                   ObjectVectorValue (),
                   MakeObjectVectorAccessor (&TricklesL4Protocol::m_sockets),
                   MakeObjectVectorChecker<TricklesSocketBase> ())
  ;
  return tid;
}

TricklesL4Protocol::TricklesL4Protocol ()
  : m_endPoints (new Ipv4EndPointDemux ()), m_endPoints6 (new Ipv6EndPointDemux ())
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_LOG_LOGIC ("Made a TricklesL4Protocol "<<this);
}
...
void 
TricklesL4Protocol::SetNode (Ptr<Node> node)
{
  m_node = node;
}
 ...
 * by setting the node in the Trickles stack, link it to the ipv4 stack and
 * adding Trickles socket factory to the node.
 */

    void TricklesL4Protocol::NotifyNewAggregatePublic () {
        NotifyNewAggregate();
    }
    
void
TricklesL4Protocol::NotifyNewAggregate ()
{
  ...
  if (m_node == 0)
    {
      if ((node != 0) && (ipv4 != 0 || ipv6 != 0))
        {
          this->SetNode (node);
          Ptr<TricklesSocketFactoryImpl> tricklesFactory = CreateObject<TricklesSocketFactoryImpl> ();
         tricklesFactory->SetTrickles (this);
          node->AggregateObject (tricklesFactory);
        }
    }
...
int 
TricklesL4Protocol::GetProtocolNumber (void) const
{
  return PROT_NUMBER;
}

void
TricklesL4Protocol::DoDispose (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_sockets.clear ();
...
Ptr<Socket>
TricklesL4Protocol::CreateSocket (TypeId socketTypeId)
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  Ptr<TricklesSocketBase> socket = socketFactory.Create<TricklesSocketBase> ();
  socket->SetNode (m_node);
  socket->SetTrickles (this);
  socket->SetRtt (rtt);
...
Ptr<Socket>
TricklesL4Protocol::CreateSocket (void)
{
  return CreateSocket (m_socketTypeId);
}

Ipv4EndPoint *
TricklesL4Protocol::Allocate (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
Ipv4EndPoint *
TricklesL4Protocol::Allocate (Ipv4Address address)
{
...
Ipv4EndPoint *
TricklesL4Protocol::Allocate (uint16_t port)
{
...
Ipv4EndPoint *
TricklesL4Protocol::Allocate (Ipv4Address address, uint16_t port)
{
...
Ipv4EndPoint *
TricklesL4Protocol::Allocate (Ipv4Address localAddress, uint16_t localPort,
...
void 
TricklesL4Protocol::DeAllocate (Ipv4EndPoint *endPoint)
{
...
Ipv6EndPoint *
TricklesL4Protocol::Allocate6 (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
Ipv6EndPoint *
TricklesL4Protocol::Allocate6 (Ipv6Address address)
{
  NS_LOG_FUNCTION (this << address);
...
Ipv6EndPoint *
TricklesL4Protocol::Allocate6 (uint16_t port)
{
  NS_LOG_FUNCTION (this << port);
...
Ipv6EndPoint *
TricklesL4Protocol::Allocate6 (Ipv6Address address, uint16_t port)
{
  NS_LOG_FUNCTION (this << address << port);
...
Ipv6EndPoint *
TricklesL4Protocol::Allocate6 (Ipv6Address localAddress, uint16_t localPort,
...
void
TricklesL4Protocol::DeAllocate (Ipv6EndPoint *endPoint)
{
  NS_LOG_FUNCTION (this << endPoint);
...
void 
TricklesL4Protocol::ReceiveIcmp (Ipv4Address icmpSource, uint8_t icmpTtl,
...
void 
TricklesL4Protocol::ReceiveIcmp (Ipv6Address icmpSource, uint8_t icmpTtl,
                            uint8_t icmpType, uint8_t icmpCode, uint32_t icmpInfo,
...
TricklesL4Protocol::Receive (Ptr<Packet> packet,
                        ...
                        Ptr<Ipv4Interface> incomingInterface)
{
  ...
  NS_LOG_LOGIC ("TricklesL4Protocol " << this
  ...
  NS_LOG_LOGIC ("TricklesL4Protocol "<<this<<" received a packet");
  ...
  if (endPoints.empty ())
    {
      if (this->GetObject<Ipv6L3Protocol> () != 0)
        {
          NS_LOG_LOGIC ("  No Ipv4 endpoints matched on TricklesL4Protocol, trying Ipv6 "<<this);
      ...
      NS_LOG_LOGIC ("  No endpoints matched on TricklesL4Protocol "<<this);
  ...
  NS_LOG_LOGIC ("TricklesL4Protocol "<<this<<" forwarding up to endpoint/socket");
                        ...
                        Ptr<Ipv6Interface> interface)
{
  ...
  if (endPoints.empty ())
    {
      NS_LOG_LOGIC ("  No IPv6 endpoints matched on TricklesL4Protocol "<<this);
      std::ostringstream oss;
      oss<<"  destination IP: ";
...
void
TricklesL4Protocol::Send (Ptr<Packet> packet,
                     Ipv4Address saddr, Ipv4Address daddr,
                     uint16_t sport, uint16_t dport, Ptr<NetDevice> oif)
{
  NS_LOG_FUNCTION (this << packet << saddr << ":" << sport << " >> " << daddr << ":" << dport);

  TcpHeader tcpHeader;
//  if (!packet->RemoveHeader(tcpHeader)) return;
  ...
  if (ipv4 != 0)
    {
      ...
      if (ipv4->GetRoutingProtocol () != 0)
        ...
        NS_LOG_LOGIC("Sending " << packet << " with route " << (*route));
...
TricklesL4Protocol::Send (Ptr<Packet> packet,
                     ...
                     uint16_t sport, uint16_t dport, Ptr<NetDevice> oif)
{
  NS_LOG_FUNCTION ("IPv6" << this << packet << saddr << daddr << sport << dport << oif);
...
//  if (!packet->RemoveHeader(tcpHeader)) return;
  ...
  Ptr<Ipv6L3Protocol> ipv6 = m_node->GetObject<Ipv6L3Protocol> ();
    NS_LOG_LOGIC("IPv6 pointer: " << ipv6);
  if (ipv6 != 0)
    {
      Ipv6Header header;
...
void
TricklesL4Protocol::SendPacket (Ptr<Packet> packet, const TcpHeader &outgoing,
...
//  outgoingHeader.SetLength (5); //header length in units of 32bit words
  ...
  if (ipv4 != 0)
    ...
    NS_FATAL_ERROR ("Trying to use Trickles on a node without an Ipv4 interface");
}
...
void
TricklesL4Protocol::SetDownTarget (IpL4Protocol::DownTargetCallback callback)
{
  m_downTarget = callback;
}

IpL4Protocol::DownTargetCallback
TricklesL4Protocol::GetDownTarget (void) const
{
  return m_downTarget;
}

void
TricklesL4Protocol::SetDownTarget6 (IpL4Protocol::DownTargetCallback6 callback)
{
  m_downTarget6 = callback;
}

IpL4Protocol::DownTargetCallback6
TricklesL4Protocol::GetDownTarget6 (void) const
{
